...
class LinkBudgetInputs:
    # X-band profiling overrides (canonical S-band defaults: 2.212 GHz,
    # 2.0 Msps, 6.0 dB margin) — this tool benchmarks a wideband payload link.
    frequency_GHz: float = 8.2
    symbol_rate_Msps: float = 300.0
    margin_dB: float = 3.0
    tx_power_dBw: float = _LB_DEFAULTS.tx_power_dBw
    tx_boresight_gain_dBi: float = _LB_DEFAULTS.tx_boresight_gain_dBi
    tx_losses_dB: float = _LB_DEFAULTS.tx_losses_dB
    tx_backoff_dB: float = _LB_DEFAULTS.tx_backoff_dB
    antenna_gain_dBi: float = _LB_DEFAULTS.tx_boresight_gain_dBi
    ...
    satellite_altitude_km: float = _LB_DEFAULTS.satellite_altitude_km
    min_gs_elevation_deg: float = _LB_DEFAULTS.min_elevation_deg
    ...
    rolloff: float = _LB_DEFAULTS.rolloff


# Ny-Ålesund site (matches the "Ny-Alesund, NO" database entry to rounding).
# Named unambiguously here so it is not confused with the SvalSat/Longyearbyen
# site that other scripts refer to as "Svalbard".
...
def run_pipeline(
    ...
    results = link_budget_math.calculate_link_budget(
        ...
        receiver_noise_figure_dB=inputs.receiver_noise_figure_dB,
        sky_background_temperature_K=inputs.sky_background_temperature_K,
...
PARAMETER_MUTATORS: List[Tuple[str, Callable[[LinkBudgetInputs], LinkBudgetInputs]]] = [
    ...
    ("Receiver NF", lambda p: replace(p, receiver_noise_figure_dB=p.receiver_noise_figure_dB + 0.2)),
    (
        "Receiver losses",
